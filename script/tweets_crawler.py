...
class TwitterSearch:
    ...
    def search(self, query):
        ...
        while response is not None and continue_search and response['items_html'] is not None and counter_num <=100:
            ...
            print ('current number of tweets',counter_num)
...
if __name__ == '__main__':
   ...
   # create a list of states
    list_of_state = ['ALABAMA','ALASKA','ARIZONA','ARKANSAS','CALIFORNIA','COLORADO','CONNECTICUT',
                    ...
                    'VERMONT','VIRGINIA','WASHINGTON',"WASHINGTON%2CD.C.",'WEST%20VIRGINIA','WISCONSIN','WYOMING']
    
        
    #list_of_state =["WASHINGTON%2CD.C."]

    '''
    counstruct a query string 
    format: query_str = 'q=Donald%20Trump%20near%3A%22' + state + '%22%20within%3A15mi%20since%3A' + list_date[0] + '%20until%3A' +list_date[1]
    '''
    ...
    for state in list_of_state:
        ...
        for i in range(0,len(list_date) - 1,1):
            print 'dowloading'
            result_file = state + list_date[i] + ".csv"
            query_str = 'DonaldTrump near:"' + state + '"within:15mi since:' + list_date[i] + ' until:' +list_date[i + 1]
            print query_str
